...
%\VignetteIndexEntry{jsr223 User Manual}
...
% Required to pass CRAN checks. Sets PDF compression.
...
\title{\normalfont jsr223: A Java Platform Integration for R with Programming Languages Groovy, JavaScript, JRuby (Ruby), Jython (Python), and Kotlin}
...
The R package \pkg{jsr223} is a high-level integration for five programming languages in the Java platform: Groovy, JavaScript, JRuby, Jython, and Kotlin. Each of these languages can use Java objects in their own syntax. Hence, \pkg{jsr223} is also an integration for R and the Java platform. It enables developers to leverage Java solutions from within R by embedding code snippets or evaluating script files. This approach is generally easier than \pkg{rJava}'s low-level approach that employs the Java Native Interface. \pkg{jsr223}'s multi-language support is dependent on the Java Scripting API: an implementation of ``JSR-223: Scripting for the Java Platform'' that defines a framework to embed scripts in Java applications. The \pkg{jsr223} package also features extensive data exchange capabilities and a callback interface that allows embedded scripts to access the current R session. In all, \pkg{jsr223} makes solutions developed in Java or any of the \pkg{jsr223}-supported languages easier to use in R.
\clearpage
...
\begin{table}[h!]
\centering
\caption{The five programming languages supported by \pkg{jsr223}.}
...
\caption{The \pkg{jsr223} package provides high-level access to the Java platform through five programming languages. Although \pkg{jrs223} uses the Java Scripting API in its implementation, users do not need to learn the API.}
\label{fig:scheme-jsr223}
\end{figure}

\hypertarget{jsr223-package-implementation-and-features-overview}{}
\subsection{\pkg{jsr223} package implementation and features overview}
...
This section includes introductory examples that demonstrate typical use cases for the \pkg{jsr223} package. For a complete overview of \pkg{jsr223} features, see the \hyperlink{feature-documentation}{Feature documentation}. Following that section, we provide details and examples for each of the \pkg{jsr223}-supported languages.
...
\paragraph{The Groovy script} The Groovy script here follows Java syntax with one exception: we provide no class. Instead, we place all of the code at the top level to be executed at once. This is merely a style choice to keep the code samples easy to follow. The script begins by importing the necessary classes.
...
Next, we convert the train and test data from R objects to the \code{DataSet} objects consumed by DL4J. We retrieve the data from the R environment using the \code{get} method of \pkg{jsr223}'s built-in \code{R} object. The R matrices are automatically converted to multi-dimensional Java arrays. These arrays are used to instantiate the \code{NDArray} objects which, in turn, are used to instantiate the \code{DataSet} objects.
...
Pulling the data from the R environment using the \code{R} object is just one convenient way to share data between R and the Java environment. It is also possible to push data from R to the Groovy environment, or to pass the data as function parameters. Note: for very large data sets it is impractical to exchange data between R and Java using \pkg{jsr223} methods. Instead, load the data on the Java side for processing using DL4J classes optimized for big data.
...
The Metropolis sampler will return two multi-dimensional arrays. We prefer the arrays to be structured in a specific order, so we change the default ordering using the code here. For the sake of brevity, we refer the reader to \hyperlink{handling-r-matrices}{Handling R matrices and other n-dimensional arrays} for details regarding array order settings.
...
\begin{table}[h!]
\centering
\caption{Benchmark timings for the Java Metropolis sampler extended in Groovy. All times are in milliseconds. The first column indicates the number of MCMC iterations computed for each of the four random walks run in parallel. The second column contains the mean runtime for the expression \code{cs\$eval()} over 40 simulations. The third column is the mean runtime for \code{cs\$eval(TRUE)} (i.e., the return value is discarded). The last column is the difference between columns two and three; hence, it roughly represents the time required to convert the Java results to R objects.}
...
The section \hyperlink{r-with-ruby}{R with Ruby} includes detailed instructions for using Ruby gems (i.e., libraries) in R. Specifically, the example shows how to generate fake entities for demonstration data sets.

The core Python language features are fully accessible via \pkg{jsr223}. However, compatibility with many common Python libraries is limited. Please see \hyperlink{r-with-python}{R with Python} for more information. That section also includes a code example that uses core Python to implement a simple HTTP server. The server uses R callbacks to generate content.
...
The JRE can be obtained from  \href{http://www.oracle.com/technetwork/java/javase/downloads/index.html}{Oracle's web site}. Select the architecture (32 or 64 bit) that matches your R installation.
...
\href{https://docs.oracle.com/javase/8/docs/technotes/guides/scripting/nashorn/}{Nashorn} is the JavaScript dialect included in Java 8 through 10. For these versions, Nashorn is included in the JRE, so no downloads are necessary to use JavaScript with \pkg{jsr223}. \textcolor{red}{Note: As of Java 11, Nashorn is deprecated. Nashorn will be removed in a future Java release.} Technical documentation and examples for Nashorn are available at \href{https://docs.oracle.com/javase/8/docs/technotes/guides/scripting/nashorn/}{Oracle's web site}. Create a JavaScript instance using
...
\hypertarget{handling-r-matrices}{text}
...
\textcolor{red}{Note: As of Java 11, Nashorn JavaScript is deprecated.} See \href{https://openjdk.java.net/jeps/335}{JEP 335}.
...
Ruby libraries and programs are distributed in a standardized package format called a \textit{gem}. We demonstrate using gems in \pkg{jsr223} with Benjamin Curtis's \href{https://github.com/stympy/faker}{\code{faker}}: a library used to produce fake records for data sets \citeyearpar{faker}.

A full installation of JRuby is required to use gems. Install JRuby and using the instructions found in \href{https://github.com/jruby/jruby/wiki/GettingStarted}{Getting Started with JRuby}. Install the \code{faker} gem using \samp{gem install faker} in a terminal.
...
All limitations and issues are managed via the GitHub issue tracker at \url{https://github.com/floidgilbert/jsr223/issues}. Be default, the tracker lists only open issues. Modify the search parameters to see limitations and workarounds deemed as closed issues.
